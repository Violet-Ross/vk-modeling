...
result_files <- list.files("throughput/results_onephase", pattern = "result_task_.*\\.csv",
                            full.names = TRUE)
...
write_csv(rmse_list, "throughput/rmse_list_vary_priors_onephase.csv")
cat(sprintf("Combined %d rows written to throughput/rmse_list_vary_priors_onephase.csv\n",
            nrow(rmse_list)))
