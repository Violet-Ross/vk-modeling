...
# Called by the SLURM array job; reads its condition from param_grid.csv
...
param_grid <- read_csv("throughput/param_grid.csv", show_col_types = FALSE)
...
compute_rmse <- function(fit, pop_params, LOD) {
  ...
  # True values of base parameters
  true_alpha <- LOD
  true_beta1 <- pop_params$mu_beta1
  true_beta2 <- pop_params$mu_beta2
  true_psi   <- pop_params$mu_psi
  
  # True epidemiological quantities
  true_peak     <- true_alpha + true_beta1 * true_psi
  true_duration <- (true_beta2 * true_psi - true_alpha) / (true_beta1 + true_beta2)
  
  # Posterior samples of epidemiological quantities (sample-wise, preserving joint structure)
  # alpha is fixed at LOD by design, not estimated, so LOD is used directly here
  post_peak     <- LOD + post$mu_beta1 * post$mu_psi
  post_duration <- (post$mu_beta2 * post$mu_psi - LOD) /
    (post$mu_beta1 + post$mu_beta2)
  
  data.frame(
    alpha_rmse    = sqrt(mean((post$mu_alpha - true_alpha)^2)),
    beta1_rmse    = sqrt(mean((post$mu_beta1 - true_beta1)^2)),
    beta2_rmse    = sqrt(mean((post$mu_beta2 - true_beta2)^2)),
    psi_rmse      = sqrt(mean((post$mu_psi   - true_psi)^2)),
    peak_rmse     = sqrt(mean((post_peak     - true_peak)^2)),
    duration_rmse = sqrt(mean((post_duration - true_duration)^2))
  )
}
...
for (iter in seq_len(N_ITER)) {
  ...
  fit  <- fit_model(all_trajectories, prior_sd, STAN_FILE, seed = iter)
  rmse <- compute_rmse(fit, pop_params, LOD)
  ...
  rows[[iter]] <- data.frame(
    ...
    peak_rmse     = rmse$peak_rmse,
    duration_rmse = rmse$duration_rmse,
    sd            = prior_sd,
    l0            = l_0,
    task_id       = task_id,
    iter          = iter
...
out_path <- sprintf("throughput/results_onephase/result_task_%04d.csv", task_id)
